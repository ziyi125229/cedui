...
function parse(raw, priorAnswer, message) {
  ...
  try {
    ...
    const grounded = arr.filter(x => {
      ...
      // The exact anchor is the grounding proof. Do not additionally require
      // lexical overlap between anchor and response: a natural reply can
      // paraphrase the anchored point without repeating its words.
...
async function generate({ user, priorAnswer, message, repair = false }) {
  const raw = await callLLM({
    ...
    user: repair
      ? `${user}\n\n【严格修正】上一版存在“重复询问已经知道的信息”或“凭空补充 TA 没说的内容”的问题。重新生成 3 条。每条必须从 TA 原话中选择一个 4-20 字的连续片段作为 anchor；anchor 必须逐字存在于 TA 原话中。澄清只能询问真正未知、且有助于推进对话的问题；如果答案已经在用户原话或 TA 原话里明确出现，就直接承接，不要再问。不要为了凑 3 条编造新的 TA 信息。`
      : user,
